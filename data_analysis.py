...
amplifier_Data = loadmat(r'C:\Users\User\Documents\PPKE ITK info-bionika\_HillierLab\FFT\amplifier161329_2.mat')
...
#plotting all of the 32 channels of electrode at the same time in order to see whether one of the channels malfunctions
...
def plotting_fft(one_channel, t_, title_, x_begin, x_end, x_label, y_label, name_save, is_control):
  ...
  xf = fft(x-x.mean()) #creating fast fourier transform of signal
...
def fft_FromLastMinute(one_channel, t_, title_, x_begin, x_end, x_label, y_label, name_save, is_control):
  x = one_channel[1200000:].reshape(-1) #creating one dimensional array
  t = np.array(t_).squeeze()
  dt = t[1]-t[0]
  #total number of data points
  N = x.shape[0]
